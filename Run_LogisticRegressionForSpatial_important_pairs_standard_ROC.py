...
def plot_results(nrow,ncol,nameOfCellType,radius,cmn,coef,classes,CTFeatures,x_test,x_train,predicted_probs,inputFeatures,inputdir,fpr, tpr, roc_auc):
	filename=str(radius)
...
def model_log_regression(no_of_times_to_run_logistic_regression,neighborhoodClass,target):
	...
	return cmn,coef,classes, CTFeatures,x_test,x_train,y_prob ,fpr, tpr, roc_auc
...
def main():
		...
		for i in radiusList:
			...
			cmn,coef,classes,CTFeatures,x_test,x_train,predicted_probs,fpr, tpr, roc_auc=model_log_regression(no_of_times_to_run_logistic_regression,neighborhoodClass,target)
			find_interacting_cell_types(cmn,coef,CTFeatures,nameOfCellType,fw)
			plot_results(2,3,nameOfCellType,radius,cmn,coef,classes,CTFeatures,x_test,x_train,predicted_probs,inputFeatures,inputdir,fpr, tpr, roc_auc)
