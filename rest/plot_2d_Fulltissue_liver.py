...
def main():
    ...
    '''
    df_cluster=pd.read_csv('Tangram/Tangram_derived_clusters.dat')
    tangram_cluster=df_cluster.to_numpy()
    df=pd.read_csv('Tangram/nameOfCT_poss3_24.dat',sep='\t',header=None)
    tangram_ctname=df.to_numpy()

    degbased_cluster=sort_index_in_right_order(degbased_cluster,tangram_cluster)
    degbased_ctname=tangram_ctname
    '''
    ...
    mycluster_interest=mycluster_interest_all
    CTname2=degbased_ctname[:,1]
    cl2=find_id(degbased_ctname,mycluster_interest,degbased_cluster)
    PP2,id2,cellsinCT2=reading_data(posdata,cl2,degbased_cluster,mycluster_interest_id[0])
    ...
    fig.tight_layout()
    fig.savefig('all_cell_types_liver.png',bbox_inches='tight',dpi=300)
