...
def sort_index_in_right_order(correct,wrong):
    ...
    for i in range(len(correct)):
        ...
        index.append(value)
    right=wrong[index]
    print('if correct should print 0',count)
    return right
...
def main():
    ...
    degbased_cluster=spatialcluster
    degbased_ctname=spatial_name
    ...
    #for i in range(7,len(mycluster_interest_all)):
    for i in range(8):
        ...
        fig.tight_layout()
        name=mycluster_interest[0].replace('/','_')
...
def plot_specific_ct(CTname,PP,index,ax):
    ...
    for j in range(len(index)):
        labelname=CTname[j]+'-'+str(len(index[j]))
