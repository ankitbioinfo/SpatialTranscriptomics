...
def find_PC_of_invidualCluster_in_SC(scbarcode,scadata,no_of_pc,spbarcode,spadata, sct_ad_sc_full,outputdir,celltype_name,threshold_for_LR_exp_in_cell_population):
    ...
    gname1a,geneNMF1a=top_genes_in_correlation_list_abs(genename_joint,corr_NMFfactors_genes1,5)
    gname1b,geneNMF1b=top_genes_in_correlation_list_without(genename_joint,corr_NMFfactors_genes1,5)

    corr_NMFfactors_genes2=find_correlation_bw_genes_and_PC_component_in_singlecell(spH.T,msp)
    gname2a,geneNMF2a=top_genes_in_correlation_list_abs(genename_spatial,corr_NMFfactors_genes2,5)
    gname2b,geneNMF2b=top_genes_in_correlation_list_without(genename_spatial,corr_NMFfactors_genes2,5)

    corr_NMFfactors_genes3=find_correlation_bw_genes_and_PC_component_in_singlecell(H.T,CbyG)
    gname3a,geneNMF3a=top_genes_in_correlation_list_abs(genename_full,corr_NMFfactors_genes3,25)
    gname3b,geneNMF3b=top_genes_in_correlation_list_without(genename_full,corr_NMFfactors_genes3,25)
...
def run_ridge_regression(input,savedir,saveoutname,ylabelname,target,neighborhoodClass,shap_cluster_cutoff):
    ...
    dir1=savedir+'FeatImpt_/'
    dir2=savedir+'Bar_/'
    dir3=savedir+'Summary_/'
    dir4=savedir+'Beeswarm_/'
    dir5=savedir+'Dependece_/'
    dir6=savedir+'SummaryInteraction_/'
    dir7=savedir+'Dependece_Interaction_/'
    create_directory(dir1)
    create_directory(dir2)
    create_directory(dir3)
    create_directory(dir4)
    create_directory(dir5)
    create_directory(dir6)
    create_directory(dir7)
    ...
    reg_alpha=[]
    lambda_c=[]
    coef=np.random.rand(input.no_of_pc,len(ylabelname))
    '''
    XGBRegressor(base_score=0.5, booster='gbtree', callbacks=None,
             ...
             num_parallel_tree=1, predictor='auto', random_state=3685138,
             reg_alpha=0, reg_lambda=1, ...)

        #dir ['_Booster', '__class__', '__delattr__', '__dict__', '__dir__', '__doc__', '__eq__', '__format__', '__ge__', '__getattribute__', '__getstate__', '__gt__',
        #'__hash__', '__init__', '__init_subclass__', '__le__', '__lt__', '__module__', '__ne__', '__new__', '__reduce__', '__reduce_ex__', '__repr__', '__setattr__',
         #'__setstate__', '__sizeof__', '__sklearn_is_fitted__', '__str__', '__subclasshook__', '__weakref__', '_can_use_inplace_predict', '_check_feature_names',
         '_check_n_features', '_configure_fit', '_early_stopping_attr', '_estimator_type', '_get_iteration_range', '_get_param_names', '_get_tags', '_get_type',
         '_more_tags', '_repr_html_', '_repr_html_inner', '_repr_mimebundle_', '_set_evaluation_result', '_validate_data', '_validate_params', 'apply',
         'base_score', 'best_iteration', 'best_ntree_limit', 'best_score', 'booster', 'callbacks', 'coef_', 'colsample_bylevel', 'colsample_bynode',
         'colsample_bytree', 'early_stopping_rounds', 'enable_categorical', 'eval_metric', 'evals_result', 'feature_importances_', 'feature_names_in_',
         'fit', 'gamma', 'get_booster', 'get_num_boosting_rounds', 'get_params', 'get_xgb_params', 'gpu_id', 'grow_policy', 'importance_type',
         'interaction_constraints', 'intercept_', 'learning_rate', 'load_model', 'max_bin', 'max_cat_to_onehot', 'max_delta_step', 'max_depth',
         'max_leaves', 'min_child_weight', 'missing', 'monotone_constraints', 'n_estimators', 'n_features_in_', 'n_jobs', 'num_parallel_tree',
         'objective', 'predict', 'predictor', 'random_state', 'reg_alpha', 'reg_lambda', 'sampling_method', 'save_model', 'scale_pos_weight',
         'score', 'set_params', 'subsample', 'tree_method', 'validate_parameters', 'verbosity']
    '''
    x_train, X_validation, y_train, y_validation = train_test_split(neighborhoodClass, target, test_size=0.05,random_state=seed)
    print(saveoutname,x_train.shape,X_validation.shape,np.sum(neighborhoodClass<0),np.sum(target<0))
    ...
    for i in range(y_train.shape[1]):
        ...
        clf = RandomizedSearchCV(estimator=xgbr,
                         ...
                         n_iter=25,
                         verbose=1)
        clf.fit(neighborhoodClass, target)
        ...
        xgb_model = xgb.XGBRegressor(booster='gbtree',objective="reg:squarederror", random_state=seed,n_estimators=100, max_depth=6, learning_rate=1,
        subsample=0.99,colsample_bytree=0.99,num_boost_round=1,eval_metric=mean_squared_log_error ,early_stopping_rounds=10,verbosity=0)
        #r2_score and explained_variance_score should not use as these minimized the explained variance score
        #    mean_squared_error  mean_squared_log_error mean_absolute_error

        #xgb_model = xgb.XGBRegressor(booster='gblinear',objective="reg:squarederror", random_state=seed)
        ...
        #xgb_model.fit(Xdata,y_train[:,i])
        xgb_model.fit(x_train, y_train[:,i], eval_set=[(x_train, y_train[:,i]), (X_validation, y_validation[:,i])],verbose=False)

        yyhat=xgb_model.predict(x_train,iteration_range=(0,xgb_model.best_iteration+1)) #ntree_limit=xgb_model.best_ntree_limit
        #https://xgboost.readthedocs.io/en/latest/python/python_intro.html#prediction
        #https://xgboost.readthedocs.io/en/stable/python/python_api.html
        #https://stackoverflow.com/questions/37627923/how-to-get-feature-importance-in-xgboost
        yhat.append(yyhat)

        print('yhat',i,np.sum(yyhat<0))

        xgb_model.get_booster().feature_names = ylabelname
        ...
        total_gain=xgb_model.get_booster().get_score(importance_type='total_gain')
        total_cover=xgb_model.get_booster().get_score(importance_type='total_cover')
        fw=open(dir1+saveoutname+'_Fa'+str(i+1)+'.dat','w')
        fw.write('Features\tgain\tweight\tcover\ttotal_gain\ttotal_cover\n')
        for j in range(len(ylabelname)):
            key=ylabelname[j]
            try:
                coef[i][j]=gain[key]
                fw.write(key+'\t'+str(gain[key])+'\t'+str(weight[key])+'\t'+str(cover[key])+'\t'+str(total_gain[key])+'\t'+str(total_cover[key])+'\n')
            except KeyError:
                coef[i][j]=0
        fw.close()


        #this line return the weight similar to above
        #feat_imp = pd.Series(xgb_model.get_booster().get_fscore()).sort_values(ascending=False)
        #print("Feat_imp",feat_imp)

        results = xgb_model.evals_result()
        #print('best iteration',xgb_model.best_iteration, 'best score= ', xgb_model.best_score, 'best ntree limit',xgb_model.best_ntree_limit)
        #coef=xgb_model.coef_
        LRI.append(0)
        #intercept=xgb_model.intercept_
        reg_lambda.append(xgb_model.reg_lambda)#'%0.2f'%
        reg_alpha.append(xgb_model.reg_alpha)#'%0.2f'%
        lambda_c.append(0)
        mse=mean_squared_error(y_train[:,i], yyhat)
        rmse=np.sqrt(mse)
        EVS=explained_variance_score(y_train[:,i],yyhat)

        #1 Feature importance plot
        xgb.plot_importance(xgb_model.get_booster())
        plt.title(saveoutname+'_'+'Fa'+str(i+1)+", rmse ="+'%0.4f'%rmse+", EVS = " +'%0.4f'%EVS)
        plt.tight_layout()
        plt.savefig(dir1+saveoutname+'_'+'Fa'+str(i+1)+'.png')
        plt.close('all')

        #2 Bar plot
        explainer = shap.Explainer(xgb_model,feature_names=ylabelname)
        shap_values = explainer(x_train)
        clust = shap.utils.hclust(x_train, y_train[:,i], linkage="single")
        shap.plots.bar(shap_values, clustering=clust, clustering_cutoff=1.1, show=False)
        plt.title(saveoutname+'_'+'Fa'+str(i+1)+", rmse ="+'%0.4f'%rmse+", EVS = " +'%0.4f'%EVS)
        plt.tight_layout()
        plt.savefig(dir2+saveoutname+'_'+'Fa'+str(i+1)+'.png',dpi=300, bbox_inches = "tight")
        plt.close('all')
        ...
        plt.title(saveoutname+'_'+'Fa'+str(i+1)+", rmse ="+'%0.4f'%rmse+", EVS = " +'%0.4f'%EVS)
        plt.tight_layout()
        plt.savefig(dir3+saveoutname+'_'+'Fa'+str(i+1)+'.png',dpi=300, bbox_inches = "tight")
        plt.close('all')


        if input.shap_computation:
            #4 beeswarm plot
            explainer = shap.TreeExplainer(xgb_model,feature_names=ylabelname)
            shap_values = explainer.shap_values(x_train)
            shap_obj=explainer(x_train)
            shap.plots.beeswarm(shap_obj,show=False)
            plt.title(saveoutname+'_'+'Fa'+str(i+1)+", rmse ="+'%0.4f'%rmse+", EVS = " +'%0.4f'%EVS)
            plt.tight_layout()
            plt.savefig(dir4+saveoutname+'_'+'Fa'+str(i+1)+'.png',dpi=300, bbox_inches = "tight")
            plt.close('all')

            #6 summary interactions plot
            X=pd.DataFrame(x_train)
            X.columns=ylabelname
            shap_interaction_values = explainer.shap_interaction_values(X)
            shap.summary_plot(shap_interaction_values,X, show=False)
            plt.title(saveoutname+'_'+'Fa'+str(i+1)+", rmse ="+'%0.4f'%rmse+", EVS = " +'%0.4f'%EVS)
            plt.tight_layout()
            plt.savefig(dir6+saveoutname+'_'+'Fa'+str(i+1)+'.png',dpi=300, bbox_inches = "tight")
            plt.close('all')

            #7 dependent interaction plot
            shap.summary_plot(shap_interaction_values, X, plot_type="compact_dot",show=False)
            plt.title(saveoutname+'_'+'Fa'+str(i+1)+", rmse ="+'%0.4f'%rmse+", EVS = " +'%0.4f'%EVS)
            plt.tight_layout()
            plt.savefig(dir7+saveoutname+'_'+'Fa'+str(i+1)+'.png',dpi=300, bbox_inches = "tight")
            plt.close('all')
            #print('A',saveoutname,shap_interaction_values[0].round(2))
            #print('B',shap_values)
            snn.heatmap(shap_interaction_values[0].round(2), xticklabels=ylabelname,yticklabels=ylabelname,annot=True)
            plt.title(saveoutname+'_'+'Fa'+str(i+1)+", rmse ="+'%0.4f'%rmse+", EVS = " +'%0.4f'%EVS)
            plt.savefig(dir7+'Heatmap_'+saveoutname+'_'+'Fa'+str(i+1)+'.png',dpi=300, bbox_inches = "tight")
            plt.close('all')


            #5 dependence plot
            mydir=dir5+saveoutname+'_'
            #create_directory(mydir)
            for fi in range(len(ylabelname)):
                shap.dependence_plot(fi, shap_values,x_train,show=False)
                plt.tight_layout()
                l=ylabelname[fi].split('_')
                plt.savefig(mydir+l[0]+'_'+l[-1]+'_.png',dpi=300, bbox_inches = "tight")
                plt.close('all')


        print(i,saveoutname,"evs=",EVS,rmse,mse)
    ...
    #print("Shape",y_train.shape,mu,yhat.shape)
    ...
    #print('\ncalc1',saveoutname,explained_ss,residual_ss, total_ss,'\t\t',percent_variance_explained,residual_variance_explained,'\t\t',reg_lambda,reg_alpha)

    pv=np.ones(coef.shape,dtype=float)
    EVS=[]
    rss=[]
    for i in range(y_train.shape[1]):
        #EVS.append(explained_variance_score(save_y_test[:,i], save_y_pred[:,i]))
        #explained variance is defined by = [1 - var(y-yhat)/var(y)]
        EVS.append(explained_variance_score(y_train[:,i], yhat[:,i]))
        rss.append(np.sum((y_train[:,i]-yhat[:,i])**2,axis=0))
        params = np.append(LRI[i],coef[i,:])
    ...
    print('\ncalc2',saveoutname, EVS, rss)



    if False:#(saveoutname=='12_LSECs')|(saveoutname=='11_KCs')|(saveoutname=='20_Stellate_cells'):
            print(saveoutname,rss,EVS)
            fw=open(savedir+'plot_Y_and_X/'+saveoutname+'_'+yname[0]+'_.dat','w')
            for i in range(x_train.shape[0]):
                fw.write(str(y_train[i,0])+','+str(y_train[i,1])+','+str(y_train[i,2])+','+str(x_train[i,0])+','+str(x_train[i,1])+','+str(x_train[i,2])+'\n')
            fw.close()

            fig, ax = plt.subplots(3,3,figsize=(10,8))
            ax[0,0].plot(x_train[:,0],y_train[:,0],'b.')
            ax[0,1].plot(x_train[:,1],y_train[:,0],'b.')
            ax[0,2].plot(x_train[:,2],y_train[:,0],'b.')

            ax[1,0].plot(x_train[:,0],y_train[:,1],'b.')
            ax[1,1].plot(x_train[:,1],y_train[:,1],'b.')
            ax[1,2].plot(x_train[:,2],y_train[:,1],'b.')

            ax[2,0].plot(x_train[:,0],y_train[:,2],'b.')
            ax[2,1].plot(x_train[:,1],y_train[:,2],'b.')
            ax[2,2].plot(x_train[:,2],y_train[:,2],'b.')
    ...
    intercept=LRI
    residual_variance_explained=rss
...
def find_canonical_pathways_in_interacting_cell_types(input,maindir,savedir,totalLRpairs):
    ...
    worksheetrow=0
    header=['Id','A','B','localized score','Fa(A)','Fa(B)', 'Coeff' ,'Ligand(A)','Receptor(B)','GeneCor(Lig)','GeneCor(Rec)','AvgExp(A)','AvgExp(B)','PopExp(A)','PopExp(B)']
    for ri in range(len(header)):
