...
from sklearn.linear_model import LogisticRegression,LogisticRegressionCV, Lasso,Ridge, RidgeCV, LinearRegression
...
def run_ridge_regression(input,savedir,saveoutname,ylabelname,target,neighborhoodClass,shap_cluster_cutoff):
    ...
    test_index=[]
    n=int(neighborhoodClass.shape[1]/input.no_of_pc)
    x_train,x_test=neighborhoodClass[train_index],neighborhoodClass[test_index]
    y_train,y_test=target[train_index],target[test_index]

    global_Coef=np.zeros((input.no_of_pc,neighborhoodClass.shape[1]),dtype=float)
    global_PV=0.5*np.ones((input.no_of_pc,neighborhoodClass.shape[1]),dtype=float)
    global_intercept=np.zeros((n,input.no_of_pc),dtype=float)
    global_lambda_c=np.zeros((n,input.no_of_pc),dtype=float)
    global_pve = np.ones((n,input.no_of_pc),dtype=float)
    global_rve=np.zeros((n,input.no_of_pc),dtype=float)
    neighborhoodClass=0
    target=0
    ...
    for fi in range(n):
        NC_CT_index=[]
        for fj in range(input.no_of_pc):
            NC_CT_index.append(input.no_of_pc*fi+fj)

        Xdata=x_train[:,NC_CT_index]
        ylabelname=np.array(ylabelname)
        yname=list(ylabelname[NC_CT_index])

        #print(Xdata.shape,y_train.shape,saveoutname,len(ylabelname),x_train.shape)
        ...
        for i in range(y_train.shape[1]):
            seed=seed+1
            xgb_model = xgb.XGBRegressor(booster='gbtree',objective="reg:squarederror", random_state=seed)
            ...
            print(yname)
            xgb_model.get_booster().feature_names = yname
            xgb.plot_importance(xgb_model.get_booster())
            plt.tight_layout()
            plt.savefig(dir1+saveoutname+'_'+'Fa'+str(i+1)+'__'+yname[0]+'.png')
            plt.close('all')

            mse=mean_squared_error(y_train[:,i], yyhat)
            rmse=np.sqrt(mse)
            EVS=explained_variance_score(yyhat,y_train[:,i])

            explainer = shap.Explainer(xgb_model,feature_names=yname)
            shap_values = explainer(Xdata)

            clust = shap.utils.hclust(Xdata, y_train[:,i], linkage="single")
            shap.plots.bar(shap_values, clustering=clust, clustering_cutoff=1.1, show=False)
            plt.title("rmse ="+'%0.4f'%rmse+", EVS = " +'%0.4f'%EVS)
            plt.tight_layout()
            plt.savefig(dir2+saveoutname+'_'+'Fa'+str(i+1)+'__'+yname[0]+'.png',dpi=300, bbox_inches = "tight")
            plt.close('all')
        ...
        print(saveoutname+'_'+yname[0],yhat.shape,percent_variance_explained,residual_variance_explained,'\t\t',reg_lambda,reg_alpha)
        ...
        #if (saveoutname=='12_LSECs')|(saveoutname=='11_KCs')|(saveoutname=='20_Stellate_cells'):
        if False:
            ...
            for i in range(len(LRC)):
                for j in range(len(LRC[i])):
                    ax[i,j].set_title('C=%0.3f'%LRC[i,j]+',I=%0.3f'%LRI[i]+',pv=%0.3f'%pv[i,j]+', lamda='+str(reg_lambda),fontsize=8)
            plt.tight_layout()
            fig.savefig(savedir+'plot_Y_and_X/'+saveoutname+'_'+yname[0]+'_.png',dpi=300, bbox_inches = "tight")
            plt.close('all')

        global_Coef[:,NC_CT_index]=coef
        global_PV[:,NC_CT_index]=pv
        global_intercept[fi]=intercept
        global_lambda_c[fi]=reg_lambda
        global_pve[fi]=percent_variance_explained
        global_rve[fi]=residual_variance_explained

    coef=global_Coef
    pv=global_PV
    intercept=global_intercept
    lambda_c=global_lambda_c
    percent_variance_explained=global_pve
    residual_variance_explained=global_rve
    EVS=0
